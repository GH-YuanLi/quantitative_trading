...
class Data_preprocessing:
	...
	def load_data(self):
		...
		df = (
			df[
				...
				(df['trade_time'] >= self.start_date) & (df['trade_time'] <= self.end_date + datetime.timedelta(days=1))
			]
			.sort_values('trade_time')
			.copy()
		)
		df['trade_time_str'] = df['trade_time'].apply(
	...
	def time_filter(self, df, flag_break=True):
		...
		def night_filter(group):
			...
			return group[~((group['trade_time'] >= start) & (group['trade_time'] <= end))]

		try:
			df_night_shift = df.groupby(df['trade_time'].dt.date, group_keys=False).apply(night_filter)

			# 创建完整时间序列
			self.dt_all = pd.date_range(start=df['trade_time'].iloc[0], end=df['trade_time'].iloc[-1], freq='1min')
			#  获取所有观测时间
			self.dt_obs = [d.strftime('%Y-%m-%d %H:%M:%S') for d in pd.to_datetime(df_night_shift['trade_time'])]
			# 获取所有缺失时间
			if flag_break:
				self.dt_breaks = [d for d in self.dt_all.strftime('%Y-%m-%d %H:%M:%S').tolist() if d not in self.dt_obs]
