...
# 设置Tushare的token
ts.set_token('d75ae646c0ca9f00cd95dabd11d8689768976d613a19a682d535d0f7')  # 替换为你的Tushare token
pro = ts.pro_api()


def get_main_and_continuous_contracts(exchange, fut_code):
	"""获取期货品种的主力与连续合约"""
	df = pro.fut_basic(exchange=exchange, fut_type='2', fut_code=fut_code, fields='ts_code,symbol,name')
...
def fetch_day_data(contract_code, trade_date, freq):
	...
	night_start = dt.datetime.strptime(trade_date, '%Y%m%d').replace(hour=21, minute=0, second=0)
	night_end = night_start + dt.timedelta(hours=5, minutes=30)  # 次日 02:30

	# 交易日 T 的白盘：09:00~15:00
	day_start = dt.datetime.strptime(trade_date, '%Y%m%d').replace(hour=9, minute=0, second=0)
...
def download_futures_minute_data(exchange, fut_code, freq, start_date, end_date):
	...
	"""
	# 1. 取主力合约（过滤掉连续合约）
	df_contracts = pro.fut_basic(exchange=exchange, fut_type='2', fut_code=fut_code, fields='ts_code,symbol,name')
	# 主力合约名称示例：'沪银主力'，连续合约示例：'沪银连续'
	main_contract_list = [
		c for c in df_contracts['ts_code'] if '连续' not in df_contracts[df_contracts['ts_code'] == c]['name'].values[0]
	]
	...
	with tqdm(total=total_days, desc='下载进度') as pbar:
		for _, row in mapping_df.iterrows():
			...
			# 夜盘 21:00–次日 02:30
			night_start = dt.datetime.strptime(trade_date, '%Y%m%d').replace(hour=21, minute=0)
			night_end = night_start + dt.timedelta(hours=5, minutes=30)

			# 白盘 09:00–15:00
			day_start = dt.datetime.strptime(trade_date, '%Y%m%d').replace(hour=9, minute=0)
			day_end = day_start + dt.timedelta(hours=6)
			...
			pbar.update(1)
			pbar.set_postfix({'日期': trade_date, '合约': real_contract, '累计': len(all_data)})
			time.sleep(0.1)

	return all_data


def save_to_csv(data, filename='xxx.csv'):
	"""保存数据到CSV文件"""
	if not data.empty:
...
def save_to_parquet(data, file_path='xxx.parquet'):
	"""保存数据到Parquet文件"""
	data['open'] = data['open'].astype(float)
	data['close'] = data['close'].astype(float)
	data['amount'] = data['amount'].astype(float)
	data['trade_date'] = data['trade_date'].astype(int)
	data['trade_time'] = data['trade_time'].astype('datetime64[ns]')
	data.to_parquet(file_path)


def downloads(fut_code = 'AG', freq = '1min', start_date = dt.datetime(2025, 8, 1), end_date = dt.datetime.now(), folder = ''):
	assert freq in ['1min', '5min', '15min', '30min', '60min'], "数据频率错误，请从 ['1min', '5min', '15min', '30min', '60min'] 中选择！"
	params = {
		'exchange': 'SHFE',
		'fut_code': fut_code,
		'freq': freq,  # 数据频率
	...
	output_file = (
		f'{params["fut_code"]}_{params["freq"]}_{params["start_date"]:%Y%m%d}_{params["end_date"]:%Y%m%d}.parquet'
	)
	...
	print(f'\t · 时间范围: {params["start_date"].strftime("%Y-%m-%d")} 到 {params["end_date"].strftime("%Y-%m-%d")}')
	...
	save_to_parquet(data, os.path.join(folder, output_file))
	print('下载完成！\n')

	# df = pd.read_parquet(os.path.join('E:/uv_project/Quantitative_trading/data', output_file), engine='pyarrow')
	# print(df.info())
